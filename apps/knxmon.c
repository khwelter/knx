...
void	iniCallback( char *_block, char *_para, char *_value) {
	debugit( 3, "iniCallback( block: %s, para: %s, value: %s)\n", _block, _para, _value) ;
	...
	debugit( 3, "iniCallback( block: %s, para: %s, value: %s); done\n", _block, _para, _value);
}
...
//			cfgDaemon	=	false ;
	 ...
	 *
	 */
	debugit( 1, "setting various signla handlers\n") ;
	...
	while (( opt = getopt( argc, argv, "D:FQ:r:vx:?")) != -1) {
		debugit( 1, "working on option := %c\n", opt) ;
		switch ( opt) {
		case    'D' :
	...
	debugit( 1, "evaluating command line options, 2nd parse; done\n") ;
	...
	debugit( 1, "shmKNXSize := %d \n", shmKNXSize) ;
	debugit( 1, "shmCRFSize := %d \n", shmCRFSize) ;
